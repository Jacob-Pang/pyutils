...
class ResourceManagerProxy:
    def __init__(self, update_end_of_usage_queue: ListProxy) -> None:
        self.update_end_of_usage_queue = update_end_of_usage_queue

    def update_end_of_usage(self, allocated_keys: dict, resource_usage: dict) -> None:
        self.update_end_of_usage_queue.append(
...
class ResourceManager (ResourceManagerProxy):
    def __init__(self, sync_manager: SyncManager) -> None:
        ...
        self.resources = dict()
        self.resource_sem = dict()
        self.queued_usage = dict()

    def add_resources(self, *resources: ResourceBase):
        for resource in resources:
            self.resources[resource.key] = resource
            self.resource_sem[resource.key] = Semaphore(1)
            self.queued_usage[resource.key] = 0
    ...
    def use_or_queue_resources(self, resource_usage: dict, resource_constraints: set) -> dict:
        ...
        for resource_key, usage in resource_usage.items():
            self.resource_sem.get(resource_key).acquire()

            if not self.resources.get(resource_key).has_free_capacity(usage + self.queued_usage
                    .get(resource_key)):
                
                resource_constraints.add(resource_key)
                self.queued_usage[resource_key] += usage

        if resource_constraints:
            for resource_key in resource_usage:
                self.resource_sem.get(resource_key).release()
        ...
        for resource_key, usage in resource_usage.items():
            allocated_keys[resource_key] = self.resources.get(resource_key).use(usage)
            self.resource_sem.get(resource_key).release()
    ...
    def use_from_queued_resources(self, resource_usage: dict, resource_constraints: set) -> dict:
        ...
        for resource_key in resource_constraints:
            ...
            if self.resources.get(resource_key).has_free_capacity(usage):
    ...
    def update(self) -> set:
        ...
        while self.update_end_of_usage_queue:
            allocated_keys, resource_usage = self.update_end_of_usage_queue.pop()

            for resource_key, allocated_key in allocated_keys.items():
                usage = resource_usage.get(resource_key)

                with self.resource_sem.get(resource_key):
                    self.resources.get(resource_key).free(allocated_key, usage)
    ...
    def as_proxy(self) -> ResourceManagerProxy:
        return ResourceManagerProxy(self.update_end_of_usage_queue)
