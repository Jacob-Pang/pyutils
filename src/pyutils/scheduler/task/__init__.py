...
class Task (WrappedFunction):
    def __init__(self, target_function: callable, key: str, *args: any, resource_usage: dict = dict(),
        ...
        visible: bool = True, delete_task_on_done: bool = False, **kwargs: any):
        """ Parameters:
            ...
            visible (bool, opt): Whether to show the task_state on task_manager.
    ...
    def __call__(self, *args, **kwargs) -> TaskState:
        ...
        try:   output = super().__call__(*args, **kwargs)
            ...
            return ExceptionState(self.key, time.time(), visible=self.visible,
                    remove_state=self.delete_task_on_done)

        return NewState(self.key, max(start_time + self.reschedule_freq, time.time()), self.visible) \
                if self.reschedule_pred(output) else \
                DoneState(self.key, visible=self.visible, remove_state=self.delete_task_on_done)
