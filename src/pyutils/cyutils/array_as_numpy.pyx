...
cdef np.ndarray _array_as_numpy(void* _array, size_t size, int dtype_num):
    cdef np.npy_intp shape[1]
    shape[0] = size

    return _ndarray_as_numpy(_array, shape, dtype_num)

cdef np.ndarray array_as_npy_int(void* _array, size_t size):
    return _array_as_numpy(_array, size, np.NPY_INT)

cdef np.ndarray array_as_npy_float(void* _array, size_t size):
    return _array_as_numpy(_array, size, np.NPY_FLOAT)

cdef np.ndarray array_as_npy_double(void* _array, size_t size):
